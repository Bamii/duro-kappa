...
// onboard...
router.post('/onboard', validator.createMerchantValidation, async (_req, res, next) => {
  try {
...
router.get('/currently-attending-to', validator.queueActionValidation, adminAuth(false), async (req: any & { user: Admin }, res, next: NextFunction) => {
  try {
...
router.post('/queue/extend', validator.advanceQueueValidation, adminAuth(true), async (req: any & { user: Admin }, res: Response, next: NextFunction) => {
  try {
    const { branchId } = req.user;
...
// SUPERADMIN JOB!
router.post('/branch/create', adminAuth(true), async (req: any & { user: Admin }, res: any, next: NextFunction) => {
  try {
...
router.get('/branch/list', adminAuth(true), async (req: any & { user: Admin }, res: any, next: NextFunction) => {
  try {
    const { merchantId } = req.user;
...
router.delete('/queue/delete', validator.queueActionValidation, adminAuth(false), async (req: any & { user: Admin }, res: any, next: NextFunction) => {
  try {
    const { branchId } = req.user;
    const { queueId } = req.body;
    const queueTendingTo = await database.getQueueById(queueId)
